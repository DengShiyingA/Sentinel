...
final class ApprovalStore {
    ...
    /// User messages sent to Claude from iPhone. Status: pending → sent on Stop.
    ...
    private func rebuildTimeline() {
        ...
        // User messages sent from iPhone
        let msgBase = sumBase + sessionSummaries.count
        for (i, msg) in userMessages.enumerated() {
            entries.append(TimelineEntry(
                id: "msg-\(msg.id)", time: msg.sentAt, order: msgBase + i,
        ...
        Task { @MainActor in
            ...
            if item.type == .stop {
                ...
                for i in self.userMessages.indices where self.userMessages[i].status == .pending {
                    self.userMessages[i].status = .sent
                }
