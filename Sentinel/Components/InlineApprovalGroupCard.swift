...
struct InlineApprovalGroupCard: View {
    ...
    /// Group children may carry an edited toolInput; pass it through to the
    /// store so the CLI receives the per-request modifiedInput.
    let onDecision: (String, Decision, [String: Any]?) -> Void
        ...
        VStack(spacing: 6) {
            ForEach(liveRequests) { request in
                InlineApprovalCard(request: request) { decision, modifiedInput in
                    ...
                    onDecision(request.id, decision, modifiedInput)
