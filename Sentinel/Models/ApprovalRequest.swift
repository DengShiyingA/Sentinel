...
struct ApprovalRequest: Identifiable, Codable, Equatable {
    ...
    let timeoutAt: Date
    /// Unified diff string for Write/Edit tools (generated by CLI)
    let diff: String?
...
struct AnyCodable: Codable, Equatable, CustomStringConvertible {
    ...
    static func == (lhs: AnyCodable, rhs: AnyCodable) -> Bool {
        // Compare by JSON representation for reliable equality
        // (String(describing:) can falsely match different types)
        guard let lData = try? JSONEncoder.sentinelEncoder.encode(lhs),
              let rData = try? JSONEncoder.sentinelEncoder.encode(rhs) else {
            return false
        }
        return lData == rData
    }
